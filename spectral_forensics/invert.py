...
class ComplexSpec:
    ...
    def times(self) -> np.ndarray:
        return librosa.frames_to_time(
            np.arange(self.D.shape[1]), sr=self.sr, hop_length=self.config.hop_length
        )
...
def apply_mask(
    spec: ComplexSpec, mask: np.ndarray, length: int | None = None
) -> np.ndarray:
    ...
    return librosa.istft(
        D_edited,
        hop_length=spec.config.hop_length,
        win_length=spec.config.n_fft,
        window=spec.config.window,
        length=length,
    )
...
def band_mask(
    ...
    if taper_hz > 0:
        band = np.clip((f - (lo_hz - taper_hz)) / taper_hz, 0, 1) * np.clip(
            ((hi_hz + taper_hz) - f) / taper_hz, 0, 1
        )
...
def spectral_gate(
    ...
    noise_frames: tuple[int, int] | None = None,
) -> np.ndarray:
    """谱减降噪掩码。
    ...
    smooth = sum(
        k[i, j] * pad[i : i + mask.shape[0], j : j + mask.shape[1]]
        for i in range(3)
        for j in range(3)
    )
    return smooth
...
def mask_from_image(
    path: str | Path, shape: tuple[int, int], invert: bool = False
) -> np.ndarray:
    ...
    img = Image.open(path).convert("L").resize((shape[1], shape[0]), Resampling.LANCZOS)
...
def spectral_convergence(
    target_mag: np.ndarray, y: np.ndarray, config: SpectroConfig
) -> float:
    ...
    """
    D = librosa.stft(
        y, n_fft=config.n_fft, hop_length=config.hop_length, window=config.window
    )
    mag = np.abs(D)
...
def image_to_magnitude(
    ...
    n_rows = 512
    img = Image.open(path).convert("L").resize((n_frames, n_rows), Resampling.LANCZOS)
...
def write_wav(path: str | Path, y: np.ndarray, sr: int, normalize: bool = True) -> Path:
    """写出波形，默认做峰值归一化避免削顶。"""
    path = Path(path)
