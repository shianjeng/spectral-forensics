...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--version", action="version", version=f"spectral-forensics {__version__}"
    )
    ...
    common.add_argument(
        "--window", default="hann", help="窗函数: hann / hamming / blackmanharris ..."
    )
    ...
    sr_ = sub.add_parser(
        "reassign", parents=[common], help="重分配谱：突破不确定性下限的锐化谱图"
    )
    sr_.add_argument(
        "--mag-top-db",
        type=float,
    ...
    sr_.add_argument(
        "--side-by-side", action="store_true", help="同时输出普通谱做对照（同窗同网格）"
    )
    sr_.add_argument("--title", default=None)

    se = sub.add_parser(
        "edit", parents=[common], help="在频域编辑再变回音频（保留原相位，几乎无损）"
    )
    ...
    ss.add_argument(
        "--frames", type=int, default=900, help="时间方向的帧数（决定时长）"
    )
    ...
    sa.add_argument(
        "--only-suspect", action="store_true", help="只列出可疑和存疑的文件"
    )
    sa.add_argument(
        "--html", metavar="FILE", help="同时导出一份可分享的 HTML 报告（含谱图缩略图）"
    )
    sa.add_argument(
        "--no-thumbnails", action="store_true", help="HTML 报告不画缩略图（大库更快）"
    )
    sa.add_argument("--seconds", type=float, default=120.0, help="每个文件分析时长")
    sa.add_argument(
        "--no-stereo-check", action="store_true", help="跳过强度立体声检测（更快）"
    )
    ...
    sk.add_argument(
        "--strict-optional", action="store_true", help="可选依赖缺失时返回非零退出码"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        audio = sio.load(
            args.input, sr=args.sr, offset=args.offset, duration=args.duration
        )
    ...
    if args.command == "poster":
        ...
        subtitle = (
            f"{spec.config.describe(audio.sr)} · "
        ...
        for n in n_ffts:
            ...
            specs.append(s)
            labels.append(
                f"n_fft={n}  Δt={cfg.time_resolution(audio.sr) * 1000:.0f}ms  "
        ...
        rea = rasterize(
            do_reassign(audio, cfg, mag_top_db=args.mag_top_db),
            n_time=nt,
            n_freq=nf,
            log_freq=log_freq,
            top_db=args.top_db,
        ...
        if args.side_by_side:
            base = rasterize(
                do_reassign(audio, cfg, mag_top_db=args.mag_top_db, enabled=False),
                n_time=nt,
                n_freq=nf,
                log_freq=log_freq,
                top_db=args.top_db,
            ...
            print(f"已生成对照图: {path}")
            print(
                f"锐度 (谱集中度): 普通 {sharpness(base):.3f} → "
...
def _run_edit(args, audio, stem: str) -> int:
    ...
    if args.denoise:
        mask = mask * spectral_gate(
            spec, reduction_db=args.reduction_db, threshold_db=args.threshold_db
        )
        applied.append(f"降噪 −{args.reduction_db:.0f} dB")
    if args.mask:
        mask = mask * mask_from_image(
            args.mask, spec.magnitude.shape, invert=args.invert_mask
        )
        applied.append(f"图像掩码 {args.mask}")

    if not applied:
        print(
            "没有指定任何编辑操作（--reject / --keep / --denoise / --mask）。",
            file=sys.stderr,
        )
        return 1
    ...
    if args.preview:
        ...
        p = render.comparison(
            [before, after], ["before", "after"], args.preview, palette=args.palette
        )
        print(f"已生成对比图: {p}")
    return 0
...
def _run_sonify(args) -> int:
    ...
    cfg = transform.SpectroConfig(kind="stft", n_fft=args.n_fft, hop_length=args.hop)
    mag = image_to_magnitude(
        args.image, args.sr, cfg, n_frames=args.frames, fmin=args.fmin, fmax=args.fmax
    )
    ...
    print(
        f"已生成音频: {path}  ({len(y) / args.sr:.1f}s, "
    ...
    if args.preview:
        ...
        sp = transform.compute(
            a,
            transform.SpectroConfig(
...
def _run_audit(args) -> int:
    ...
    if getattr(args, "html", None):
        ...
        path = write_report(
            results, args.html, args.input, thumbnails=not args.no_thumbnails
        )
        print(f"已生成报告: {path}")

    if args.json:
        print(_json.dumps([r.as_dict() for r in results], ensure_ascii=False, indent=2))
        return 0
    ...
    mark = {"clean": "✓", "likely": "?", "suspect": "⚠", "lossy": "·", "unknown": "!"}
