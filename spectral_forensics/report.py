...
def _thumbnail(path: Path, cutoff_khz: float | None,
               sr: int = 44100, seconds: float = 60.0) -> str | None:
...
def _row(r: AuditResult, thumb: str | None) -> str:
    ...
    img = (f'<img src="{thumb}" alt="long-term spectrum of {name}">'
           if thumb else '<div class="nothumb">no spectrum</div>')
...
def build_report(results: list[AuditResult], root: str | Path,
                 thumbnails: bool = True,
                 thumb_limit: int = 200) -> str:
    """生成完整的 HTML 文本。"""
...
def write_report(results: list[AuditResult], out_path: str | Path,
                 root: str | Path, **kwargs) -> Path:
