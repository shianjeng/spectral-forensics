...
クリックするとその位置から再生する。サンプルはワンクリックで 3 種類読み込める：
本物の FLAC、同じクリップを 128 kbps の mp3 に通したもの、そして窓長の違いを
見比べるための合成信号（スイープ・純音・クリック）だ。それぞれ直接リンクでき、
たとえば [`?sample=mp3`](https://shianjeng.github.io/spectral-forensics/?sample=mp3&lang=ja)
で開く。ページは英語と日本語に対応している（`?lang=ja` で日本語固定）。
